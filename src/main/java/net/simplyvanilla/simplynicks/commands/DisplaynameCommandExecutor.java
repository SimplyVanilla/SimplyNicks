...
public class DisplaynameCommandExecutor implements CommandExecutor {
    public boolean onCommand(@NotNull CommandSender sender, @NotNull Command command, @NotNull String label, String[] args) {
        ...
        String nick = SimplyNicks.getCache().getNick(player.getUniqueId().toString());
        if (nick == null) {
            MessageUtil.sendMessage(sender, "messages.error.nickNotFound");
            return true;
        }

        messageReplacements.put("displayname", nick);

        MessageUtil.sendMessage(sender, "messages.displayname", messageReplacements);
